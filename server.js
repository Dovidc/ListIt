...
app.post(
  ...
  try {
    ...
    if (row) {
      eventBus.publish(EVENTS.PUSH_NEARBY_LISTING, { listing: row });
    }
...
app.post('/api/conversations/:id/messages', auth, writeLimiter, validateBody(validateSendMessageRequest), async (req, res) => {
  ...
  try {
    ...
    eventBus.publish(EVENTS.REALTIME_MESSAGE, wsMessage);

    if (wsMessage.recipient_id && wsMessage.recipient_id !== req.user.id) {
      ...
      eventBus.publish(EVENTS.PUSH_NEW_MESSAGE, {
        recipientId: wsMessage.recipient_id,
        payload: {
          type: 'new_message',
          conversation_id: id,
...
// WebSocket handling moved to the dedicated realtime service
