(() => {
  function createSupporterComponents({ React, ReactDOM } = {}) {
    ...
    function SupporterBadge({
      ...
      const computedTitle = title || (since ? `Trovelr supporter since ${formatSinceLabel(since) || since}` : 'Trovelr supporter');
      const classes = [
        'supporter-badge',
      ...
      return H(Component, sharedProps,
        badgeIcons.sparkles(),
        H('span', { className: 'supporter-badge__label' }, 'Trovelr Supporter')
      );
    }
        ...
        H('div', { className: 'supporter-modal__overlay', onClick: handleOverlay },
          H('div', {
            ...
            isPrompt
              ? H('p', { className: 'supporter-modal__body' },
                  donationText
                    ? `Chip in ${donationText} once to unlock a dazzling golden badge on every listing you share.`
                    : 'Chip in once to unlock a dazzling golden badge on every listing you share.'
                )
              : H('p', { className: 'supporter-modal__body' },
