...
- All targets import `Config/Signing.xcconfig`, which seeds default bundle identifiers and the shared development team. `project.yml` now sets each product's bundle identifier to `$(LISTIT_BUNDLE_PREFIX).$(PRODUCT_NAME:rfc1034identifier)`, so overriding the prefix once (for example to `com.lisit`) automatically flows to every framework, test bundle, and future `xcodegen generate` run without manual edits.
