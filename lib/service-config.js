...
function validateConfig(config) {
  ...
  if (config.IS_PROD) {
    ...
    if (!config.STRIPE_SECRET_KEY || !config.STRIPE_WEBHOOK_SECRET) {
      errors.push('STRIPE_SECRET_KEY and STRIPE_WEBHOOK_SECRET are required in production.');
    }

    if (!config.SENDGRID_API_KEY || !config.SENDGRID_FROM_EMAIL) {
      errors.push('SENDGRID_API_KEY and SENDGRID_FROM_EMAIL are required in production.');
    }

    const hasS3Region = !!(config.S3_REGION || config.AWS_REGION);
    if (!config.S3_BUCKET || !config.AWS_ACCESS_KEY_ID || !config.AWS_SECRET_ACCESS_KEY || !hasS3Region) {
      errors.push('S3 configuration (S3_BUCKET, AWS_REGION/S3_REGION, AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY) is required in production.');
    }
  }
