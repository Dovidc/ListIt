...
function getServiceConfig() {
  ...
  const defaultMessageBusType = isProd ? 'redis' : 'memory';
  const messageBusType = (process.env.MESSAGE_BUS_TYPE || defaultMessageBusType).toLowerCase();
  ...
  return {
    ...
    // Message bus (Redis)
    REDIS_URL: redisUrl,
    MESSAGE_BUS_TYPE: messageBusType, // 'memory', 'redis'
...
function validateConfig(config) {
  ...
  const allowedBusTypes = ['memory', 'redis'];
  if (!allowedBusTypes.includes(config.MESSAGE_BUS_TYPE)) {
    errors.push('MESSAGE_BUS_TYPE must be either "memory" or "redis"');
  }

  if (config.IS_PROD) {
    if (config.MESSAGE_BUS_TYPE !== 'redis') {
      errors.push('MESSAGE_BUS_TYPE must be set to "redis" in production');
    }
    if (!config.REDIS_URL) {
