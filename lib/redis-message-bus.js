...
class RedisMessageBus extends EventEmitter {
  constructor(options = {}) {
    ...
    this._closed = false;
    this.nodeId = options.nodeId || process.env.INSTANCE_ID || os.hostname?.() || 'redis-bus-node';
  }
  ...
  async healthCheck() {
    try {
      ...
      return { ok: true, type: 'redis', nodeId: this.nodeId };
    } catch (err) {
      return { ok: false, type: 'redis', error: err?.message || err, nodeId: this.nodeId };
    }
  }
