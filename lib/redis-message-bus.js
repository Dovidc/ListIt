...
class RedisMessageBus extends EventEmitter {
  constructor(options = {}) {
    ...
    this.namespace = (options.namespace || process.env.MESSAGE_BUS_NAMESPACE || '').trim();
  ...
  _channelForTopic(topic) {
    if (!this.namespace) return topic;
    return `${this.namespace}:${topic}`;
  }

  _topicFromChannel(channel) {
    if (this.namespace && channel.startsWith(`${this.namespace}:`)) {
      return channel.slice(this.namespace.length + 1);
    }
    return channel;
  }
  ...
  _wireSubscriber() {
    ...
    this.subscriber.on('message', (channel, payload) => {
      ...
      this._emitToHandlers(topic, parsed);
    });
  ...
  async _ensureSubscription(topic) {
    ...
    try {
      await this.ready;
      const channel = this._channelForTopic(topic);
  ...
  async _removeSubscription(topic) {
    const channel = topic.includes(':') && topic === this._channelForTopic(this._topicFromChannel(topic))
      ? topic
      : this._channelForTopic(topic);
