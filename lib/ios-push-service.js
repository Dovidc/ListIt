...
function initFirebaseAdmin() {
  ...
  try {
    ...
    // Try env var JSON first (for Render/Heroku/etc)
    if (FIREBASE_SERVICE_ACCOUNT_JSON) {
      try {
        ...
        console.log('[push] Firebase Admin initialized from env var, project:', serviceAccount.project_id);
        return true;
      } catch (parseErr) {
        console.error('[push] Failed to parse FIREBASE_SERVICE_ACCOUNT_JSON:', parseErr?.message);
      }
    }
    ...
    try {
      ...
      console.log('[push] Firebase Admin initialized with service account file, project:', serviceAccount.project_id);
      return true;
    } catch (fileErr) {
