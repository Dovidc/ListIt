...
class RedisJobQueue extends EventEmitter {
  ...
  async enqueue(job) {
    ...
    await this.redis.multi()
      ...
      .lpush(this.pendingKey, payload.id)
      .exec();
    return payload.id;
  }
  ...
  async reserveNext() {
    ...
    await this.redis.zadd(this.processingTimestampsKey, Date.now() + this.visibilityTimeoutMs, job.id);
    return job;
  }
  ...
  async _promoteDelayed() {
    ...
    ids.forEach((id) => {
      ...
      pipeline.lpush(this.pendingKey, id);
    });
    await pipeline.exec();
  }
