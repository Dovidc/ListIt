...
class InMemoryQueue extends EventEmitter {
  ...
  async enqueue(job) {
    ...
    // Use setImmediate to ensure job processing starts in the next event loop tick,
    // making it independent of the current HTTP request lifecycle
...
function createWorkerQueue(options = {}) {
  ...
  if (isProd) {
    throw new Error('[worker-queue] Redis client is required in production; set REDIS_URL and install ioredis.');
  }
  return new InMemoryQueue(options);
}
