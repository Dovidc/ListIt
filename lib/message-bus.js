...
function createMessageBus(options = {}) {
  ...
  const allowInMemory = nodeEnv === 'development' || nodeEnv === 'test';
  const defaultType = allowInMemory ? 'memory' : 'redis';
  const type = (options.type || process.env.MESSAGE_BUS_TYPE || defaultType).toLowerCase();

  if (!['memory', 'redis'].includes(type)) {
    throw new Error(`Unsupported MESSAGE_BUS_TYPE "${type}". Expected "memory" or "redis".`);
  }

  if (type === 'memory') {
    if (!allowInMemory) {
      throw new Error('In-memory message bus is not allowed outside development or test environments. Configure Redis by setting MESSAGE_BUS_TYPE=redis and REDIS_URL.');
    }
    return new MessageBus(options.name || 'memory-bus');
  }

  const redisUrl = options.redisUrl || process.env.REDIS_URL || (allowInMemory ? 'redis://localhost:6379' : null);
  if (!redisUrl) {
    throw new Error('REDIS_URL must be set when using MESSAGE_BUS_TYPE=redis outside development/test.');
  }

  if (!RedisMessageBus.isSupported()) {
    const err = new Error('ioredis is required for Redis-backed message bus. Install ioredis or switch to NODE_ENV=development for local memory fallback.');
    if (!allowInMemory) {
      throw err;
    }
    console.warn('[MessageBus] Redis requested but unavailable in dev/test; falling back to in-memory bus');
    return new MessageBus(options.name || 'memory-bus');
  }

  try {
    return new RedisMessageBus({ ...options, redisUrl });
  } catch (err) {
    if (!allowInMemory) {
      throw err;
    }
    console.error('[MessageBus] Failed to initialize Redis bus in dev/test, falling back to in-memory:', err);
    return new MessageBus(options.name || 'memory-bus');
  }
}
