...
- [ ] Add reusable SwiftUI component variants (brand headers, masonry grids, modal cards, pill filters) that echo key web visuals, enabling native screens to opt into familiar patterns without bespoke styling. _(Brand header now lives in the shared design system as `ListItBrandHeader`.)_
