...
- [ ] Add reusable SwiftUI component variants (brand headers, masonry grids, modal cards, pill filters) that echo key web visuals, enabling native screens to opt into familiar patterns without bespoke styling.
  - [x] Brand header view that mirrors the Creegslist badge, title, and tagline using shared design tokens.
  - [ ] Masonry grid layout for photo-dominant surfaces such as Nearby.
  - [ ] Modal card chrome with shared corner radius, spacing, and drop shadow recipes.
  - [ ] Pill filter controls that map to the web tab affordances and filter chips.
